...
export default function Header() {
  ...
  return (
    <header className='relative h-[10vh]'>
      <div aria-label='header-wrapper' className={`flex justify-between relative px-5 py-2  z-20 bg-prim-bg ${ (toggleMenu && (vpWidth < 640))? 'bg-white' : 'bg-prim-bg' } transition-all duration-500 ease-in`} >
        <figure className='flex max-w-14 items-center h-full gap-x-2' >
          <Image 
            src={'/leton-logo.png'}
            alt='Leton Logo'
            width={500}
            height={500}
          />
          <div className='text-4xl font-inter text-[#0A1F45] tracking-wider font-bold' >
            Leton
          </div>
        </figure>
        <section className='min-w-[80%]'>
          ...
          <div aria-label='profile-section' className='min-w-[30%] justify-self-end w-fit' >
            <div aria-label='profile-section-wrapper' className='w-full flex justify-end items-center gap-x-3' >
              <div aria-label='notifications' className='relative p-2.5 rounded-lg cursor-pointer hover:bg-prim-hover hover:shadow-sm border border-slate-300/75 shadow-prim-hover' >
              ...
              <div aria-label='inbox' className='relative p-2.5 rounded-lg cursor-pointer hover:bg-prim-hover hover:shadow-sm border border-slate-300/75 shadow-prim-hover' >
                <div aria-label='inbox-container' className='relative' >
                  <div className='
                        absolute top-0 right-0 -translate-y-[25%] translate-x-[25%] bg-red-500 rounded-full w-3.5 h-3.5 text-[7px]
                        flex justify-center items-center text-white font-bold
              ...
              <button className='cursor-pointer' id='desktop-menu-btn' onClick={handleMenuToggle} >
                <figure className='flex items-center px-3 py-1 border gap-x-2 rounded-lg border border-slate-300/75 hover:bg-prim-hover hover:shadow-sm' >
                  <Image 
                    src={'/default_photo.png'}
                    alt='Profile Picture'
                    width={100}
                    height={100}
                    className='rounded-full max-w-9'
                  />
      ...
      <div aria-label='desktop-menu' className={`absolute z-19 left-0 sm:top-[4rem] w-full flex justify-end sm:px-5 ${ toggleMenu? 'translate-y-0 sopacity-100' : '-translate-y-[35rem] sopacity-0' } transition-all duration-500 ease-in`} >
        <div aria-label='desktop-menu-wrapper' id='desktop-menu' className={`w-full sm:w-[30%] bg-white border-slate-300/75 py-4 ${ (toggleMenu && (vpWidth < 640))? 'rounded-ee-lg rounded-es-lg border-b' : 'rounded-lg border' } shadow-2xl sm:shadow-md`} >
          <div className='font-semibold text-md font-inter px-5' >
            Account
          </div>
          ...
          <ul aria-label='desktop-menu-list' className='text-xs font-inter' >
              <li className='w-full mb-1 px-5 hover:bg-prim-hover/60 py-0.5' >
